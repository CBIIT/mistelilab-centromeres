...
Create a list of hits. Positive and negative sgRNA controls are not in the list of hits. Hits are defined as sgRNAs that have a z-score greater than 2.5 or less than -2.5 for Ripley's K or for the spots number for either cell line. Hits that have a cell number z-score lower than -2 are not included in the list of hits. Write the list of hits to a csv file.
...
```{r}
#| label: cell-n-hct116-rpe1-1

all_tbl_wide |>
  mutate(
    marker = ifelse(
      control == "sample" & 
      score_cell_n_HCT116 > -2.5 & 
      score_cell_n_RPE1 > 2.5,
      gene_symbol,
      ""
    )
  ) |>
  plot_cell_lines(
    x_variable = score_cell_n_HCT116,
...
all_tbl_wide |>
  ...
  plot_cell_lines(
    x_variable = score_area_mean_HCT116,
...
```{r}
#| label: area-hct116-rpe1-2

all_tbl_wide |>
  mutate(
    marker = ifelse(
      control == "sample" & 
      score_area_mean_HCT116 < 3 & 
      score_area_mean_RPE1 > 3,
      gene_symbol,
      ""
    )
  ) |>
...
```{r}
#| label: area-hct116-rpe1-3

all_tbl_wide |>
  mutate(
    marker = ifelse(
      control == "sample" & 
      score_area_mean_HCT116 < 5 & 
      score_area_mean_RPE1 < -2.5,
      gene_symbol,
      ""
    )
...
```{r}
#| label: spots-n-hct116-rpe1-1

all_tbl_wide |>
  mutate(
    marker = ifelse(
      control == "sample" & 
      score_spots_number_mean_HCT116 < -5 & 
      score_spots_number_mean_RPE1 < -5,
      gene_symbol,
      ""
    )
  ) |>
...
all_tbl_wide |>
  mutate(
    marker = ifelse(
      control == "sample" & 
      score_spots_number_mean_HCT116 > 2.5 & 
      score_spots_number_mean_RPE1 > -10,
      gene_symbol,
      ""
    )
  ) |>
...
```{r}
#| label: k-corr-perc--hct116-rpe1-1

all_tbl_wide |>
  mutate(
    marker = ifelse(
      control == "sample" & 
      score_k_corr_perc_mean_HCT116 > 3 & 
      score_k_corr_perc_mean_RPE1 > 3,
      gene_symbol,
      ""
    )
  ) |>
...
```{r}
#| label: k-corr-perc--hct116-rpe1-2
all_tbl_wide |>
  mutate(
    marker = ifelse(
      control == "sample" & 
