...
common_hits_tbl <- all_hits_tbl |>
  filter(
    ((abs(score_spots_number_mean_HCT116) > hit_cutoff) & (abs(score_spots_number_mean_RPE1) > hit_cutoff)) |
      ((abs(score_k_corr_perc_mean_HCT116) > hit_cutoff) & (abs(score_k_corr_perc_mean_RPE1) > hit_cutoff))
  ) |>
  arrange(desc(score_k_corr_perc_mean_HCT116))
