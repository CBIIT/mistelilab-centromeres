...
OBJECT_DIR <- here("Epiplus_sgrna_screen_RPE1", "hitips_input")

SYNTHEGO_DIR <- here("Epiplus_sgrna_screen_RPE1", "reformat_metadata", "Synthego")
...
if (!file_exists(fs::path(METADATA_DIR, "Description.txt"))) {
...
layout_tbl <- read_csv(
  ...
  select(
    file_name, synthego_barcode, mother_barcode, source_barcode,
...
cell_tbl <- dir_info(OBJECT_DIR,
  ...
  arrange(
    file_name, plate_index, column, row, field_index,
...
write_tsv(annotation_tbl,
  file = fs::path(METADATA_DIR, "Annotation.txt")
)
```
